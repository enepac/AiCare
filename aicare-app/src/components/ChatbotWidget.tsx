...
export default function ChatbotWidget({
  ...
  useEffect(() => {
    ...
    // Notify dashboard to scroll too
    if (onNewMessageScroll) {
      onNewMessageScroll();
    }
  }, [messages, onNewMessageScroll]);
  ...
  return (
    <div className="flex w-full h-full min-h-0 border rounded-lg overflow-hidden">
      {/* Threads Sidebar */}
      <div
        className={clsx(
          "relative border-r p-4 transition-all duration-300 flex-shrink-0 h-full overflow-y-auto",
          collapsed ? "w-0" : "w-64"
        )}
      >
        <button
          onClick={() => setCollapsed(!collapsed)}
          className="absolute -right-2 top-2 w-5 h-5 text-xs bg-white border rounded shadow hover:bg-gray-100 z-10"
      ...
      <div className="flex-1 flex flex-col h-full">
        ...
        >
          ...
          <textarea
            ...
            rows={1}
            className="flex-1 resize-none overflow-hidden px-4 py-2 border rounded focus:outline-none"
